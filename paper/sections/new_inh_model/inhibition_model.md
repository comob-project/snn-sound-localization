...
While the ITD value is obtained realistically considering the interaural path difference, which is the difference between the paths travelled by the sound starting from the source and arriving at the two ears and dividing it by the speed of sound in air (i.e., 330 m/s), the ILD value is more artificial, since it is computed as a difference in the number of spikes between left and right auditory nerve fibers (ANFs). This strategy, although very efficient in modelling the effect of ILD on various neural populations, does not take into account the natural variability of the ILD value in relation to the frequency of the input sound and its distance from the subject. Nevertheless, these aspects were not a problem with regard to the results presented in this appendix, as only one frequency of input sound was used and a fixed source distance was kept.
