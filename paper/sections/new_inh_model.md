...
- Jeffress model existence in mammals is not sure: 3 main critical issues
  ...
  - experiments with inhibition blocked (glycine antagonist strychnine) show loss of peak-shift see 10.1523/JNEUROSCI.1660-08.2008
...
Two different strategies were used to model the principal cells of LSO and MSO. Regarding the LSO, the number of cells was kept equal to that of GBCs. For the MSO, on the other hand, a battery of ten neurons was created for each frequency channel. These neurons have the same physiological characteristics and inputs, i.e., an excitation from the ipsilateral and contralateral SBCs and a dominant inhibition from the contralateral GBCs. The parameter that has been differentiated between the units of a battery is only the synaptic weight of inhibition, which takes values ranging from -2 to -24 nS. The aim is to have different slopes and distinct values in the responses of MSO neurons for different azimuth angles so that each neuron can code for the entire physiological range of ITD, as described by the two-channel model. 
All neuronal populations just described, except for ANFs, were modelled with the `iaf_cond_alpha`, an implementation of a spiking neuron in NEST using integrate-and-fire dynamics with conductance-based synapses.
...
Finally, six different configurations were tested, consisting of the following:
